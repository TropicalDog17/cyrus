...
### Added
- **Optionally run the agent's code searches on a cheaper model.** A new optional `claudeSubagentModel` setting (in `config.json`) registers a read-only `explore` subagent pinned to the model you name — for example `"claudeSubagentModel": "haiku"` — which the agent can hand broad codebase searches to instead of reading every file into the main conversation. Delegating is not automatically cheaper: a subagent re-sends its own accumulated context on every turn just like the main session, so one that inherits the session's model (Opus) mostly *moves* the cost rather than removing it. Pinning the search work to a cheaper model is what makes it pay — Opus 4.8 re-reads cached context at $0.50 per million tokens against Haiku 4.5's $0.10. When unset, nothing changes. (Claude runner only.)
- **Configurable timeout for agent questions.** A new optional `askUserQuestionTimeoutMinutes` setting (in `config.json`) controls how long Cyrus waits for your answer when the agent asks a question before unblocking it with a "no response" denial. Defaults to 30 minutes when unset; set `0` to wait indefinitely. Applies to questions asked after a config change — no restart needed. (Claude runner only.) ([DEV-144](https://linear.app/devtrop/issue/DEV-144), [#30](https://github.com/TropicalDog17/cyrus/pull/30))
- **Opt-in read access to a repository's parent directory.** A new per-repository `readParentDirectory` setting (in `config.json`) lets a session read every folder sitting next to the repo checkout — sibling repos, a shared requirements spec, a canonical schema kept one level up — instead of being limited to the repo itself. It's off by default, so the usual protection against reading anything outside the working directory (SSH keys, credentials, unrelated projects) stays fully in place for everyone who doesn't enable it. When enabled, the access is strictly read-only: the session still can only write inside its own worktree. ([DEV-139](https://linear.app/devtrop/issue/DEV-139/not-accessible-issue), [#25](https://github.com/TropicalDog17/cyrus/pull/25))
- **Follow-up comments are now far cheaper.** Cyrus keeps a finished Claude session alive for 50 minutes (configurable via `claudeSessionKeepAliveMinutes`; `0` disables) so that a follow-up comment continues the live conversation instead of restarting it. Previously the session's process shut down as soon as it answered, and the next comment had to replay the entire conversation into a fresh process — on a long issue that meant re-sending 150k–250k tokens, around a dollar, per comment. The window sits under the one-hour cache expiry, past which the saving disappears anyway. Two visible consequences: an idle issue holds a background process until the window elapses, and a first "stop" comment now interrupts the session rather than killing it outright (send "stop" again within 10 seconds to force it down). ([#22](https://github.com/TropicalDog17/cyrus/pull/22))
- **Optional cap on how many kept-alive sessions stay idle at once.** A new `claudeMaxWarmIdleSessions` setting limits the number of Claude sessions held open waiting for a follow-up; when exceeded, the least-recently-used idle session shuts down gracefully (its next comment resumes normally), and sessions with scheduled work pending are never dropped. Defaults to unbounded, where the keep-alive window alone limits accumulation. Useful if you run many concurrent issues and want a hard ceiling on held background processes. (Claude runner only.)
- **Conversation compaction is now visible on the issue.** When a session compacts its history, Cyrus posts what it cost you — for example `Compacted conversation: 210k → 45k tokens (auto)` — instead of a vague "Conversation history compacted". Compactions also show up as events on the session's Langfuse trace, so a sudden drop in a turn's token count has a visible cause. This is what makes the `claudeAutoCompactWindow` setting verifiable: a session that never compacts pays to re-read its whole history on every turn. ([#21](https://github.com/TropicalDog17/cyrus/pull/21))
- **Cap runaway context cost on long issues.** A new optional `claudeAutoCompactWindow` setting (in `config.json`) tells Claude sessions to auto-compact their conversation once it grows past the given size in tokens — for example `"claudeAutoCompactWindow": 120000`. Long issues that run many follow-ups and subroutines otherwise accumulate a large conversation that is re-sent on every turn (the dominant cost driver), because the model's built-in compaction only kicks in near its full context window. Setting a smaller window makes Cyrus compact much earlier, keeping per-turn cost and latency down. When unset, behavior is unchanged. (Claude runner only.) ([#18](https://github.com/TropicalDog17/cyrus/pull/18))
- **LLMOps observability via Langfuse.** Cyrus can now export a full trace of every agent session — each model request (model, token usage, prompt, response) and tool call (input + output) — to [Langfuse](https://langfuse.com/) for LLM observability. The export runs once at session end via an internal hook (not OTLP, which Claude Code emits as logs/metrics that Langfuse cannot ingest). Turn it on by setting `LANGFUSE_PUBLIC_KEY` and `LANGFUSE_SECRET_KEY` (and optionally `LANGFUSE_HOST`) in your environment; when unset, nothing changes, and you can force it off with `CYRUS_TELEMETRY_DISABLED=1`. See [docs/CONFIG_FILE.md](docs/CONFIG_FILE.md#llmops-observability-langfuse). ([DEV-113](https://linear.app/devtrop/issue/DEV-113/setup-llmops-for-cyrus-agent-and-langfuse), [#11](https://github.com/TropicalDog17/cyrus/pull/11), [#13](https://github.com/TropicalDog17/cyrus/pull/13))
...
### Changed
- **Claude sessions start with a much smaller tool and instruction context.** The rarely used local `cyrus-tools` server is now loaded on demand, eight census-confirmed unused Linear tools are removed from the eager Linear surface, and the unconditional failure-mode reporting addendum is no longer injected into every turn. Builder, debugger, scoper, and fallback work-management instructions were also condensed to remove repeated task-orchestration boilerplate while preserving implementation, debugging, testing, and handoff requirements. Task lifecycle tools and both `Agent`/`Task` delegation variants remain explicitly available for SDK-version compatibility. ([#32](https://github.com/TropicalDog17/cyrus/pull/32))
...
- Cyrus now also nudges the agent to hand broad codebase searches — reconnaissance, tracing a call path, "where does this live" — to a subagent that reports back a short answer with file references, rather than reading every candidate file into the main conversation, and to re-orient that way after its history is compacted instead of re-reading the same files one by one. Files it is about to edit are still read directly. In one traced session the agent read a single file 13 times, mostly to recover context it had lost to compaction.

### Security
- Claude now fails closed when an unexpected tool reaches the permission callback instead of approving every non-question tool. The hosted `Agent` delegation name is also included alongside the locally reported `Task` name. ([#32](https://github.com/TropicalDog17/cyrus/pull/32), [#33](https://github.com/TropicalDog17/cyrus/pull/33))
...
### Fixed
- **"Tool Bash is not allowed in this session" on a config that never restricted any tools.** If `linearAllowedTools` was not set — the default for most installs — the CLI passed an *empty* list rather than nothing at all, and an empty list is not the same as an absent one: it was accepted as a deliberate allow-list of zero tools and used in place of the Linear default, so sessions started with nothing pre-approved. This went unnoticed while the permission callback still approved whatever reached it; once that callback began failing closed, the same sessions started denying `Bash` (and would have denied `Edit`, `Write`, and MCP calls) with no restriction configured anywhere. An unset or empty allow-list now means "use the platform default", which is what the GitHub path has always done. ([#32](https://github.com/TropicalDog17/cyrus/pull/32))
- **Failed Langfuse exports are no longer silently reported as successful.** The Langfuse SDK resolves its flush promise even after a batch exhausts its built-in retries; Cyrus now captures the SDK's terminal ingestion errors and routes them through its structured `langfuse_export_failed` event instead of logging a false success. ([#34](https://github.com/TropicalDog17/cyrus/pull/34))
- Claude sessions retain their built-in `Bash`, file, skill, and editing tools while enforcing tool permissions. Previously, translating the permission allow-list into the SDK base-tool override could launch Claude with `--tools ""`, leaving only MCP tools available. ([#33](https://github.com/TropicalDog17/cyrus/pull/33))
- **Langfuse no longer under-charges cached prompts.** Cyrus writes its prompt cache with a 1-hour lifetime (the idle keep-alive window depends on it), which Anthropic bills at twice the base rate — but the trace reported those writes under a field Langfuse prices at the *5-minute* rate, so every session's cache writes were charged at 62.5% of what they actually cost. Cache writes are now reported split by lifetime, which Langfuse already knows how to price. Across this fork's existing sessions that accounted for **$181 of real spend that never appeared in any trace** — considerably more than the subagent gap below. Historical traces can be corrected by re-exporting; cost totals will rise accordingly, because they were wrong before, not because anything got more expensive.
- **Langfuse traces no longer under-report what a session cost.** When the agent delegated work to a subagent, everything that subagent did was missing from the session's trace — its model requests, its tool calls, and all of its tokens. Claude Code writes subagent work to separate transcript files that the exporter never opened, so a delegating session looked cheaper than it was: across recent runs on this fork, the traces omitted 13% of true cost on average and up to 41% on individual sessions. Subagent turns now appear on the trace, nested under the `Agent` tool call that spawned them, and their tokens count toward the session's cost. Existing traces are not retroactively corrected.
- **Linear tools are ready immediately instead of after a ~minute of searching.** When a session started, the agent often spent close to a minute running "tool search" round-trips against Linear's remote MCP server before it could read or update the issue — a visible stall at the top of every session. That happened because the SDK, seeing the large number of Linear tools, deferred them all behind an on-demand search to save context. Cyrus now loads the core Linear issue/comment tools up front, so they're available from the first turn with no search step. To keep that from bloating every turn, the verbose, rarely-used Linear tools (releases, milestones, attachments, diffs, documents, agent skills, cycles, project status updates, and census-confirmed unused read helpers) are pruned from the tool set. The local `cyrus-tools` server and rarely-used remote servers stay deferred to keep startup context lean. ([DEV-140](https://linear.app/devtrop/issue/DEV-140/linear-search-functionality-is-not-straightforward), [#29](https://github.com/TropicalDog17/cyrus/pull/29))
- **A single comment no longer starts several agent runs at once.** When Linear redelivered a webhook (which it does whenever the first delivery isn't acknowledged quickly), Cyrus treated the redelivery as a brand-new prompt and started a second agent process for the same comment — interrupting the first mid-work and re-sending the whole conversation to the model, at real cost on long issues. Repeat deliveries of the same comment are now ignored, and two prompts arriving at once for the same issue are handled one after the other rather than racing. A repeated "stop" is likewise no longer mistaken for the deliberate second stop that force-kills a session. ([#20](https://github.com/TropicalDog17/cyrus/pull/20))
- **`claudeAutoCompactWindow` no longer looks like it's working when it isn't.** Claude Code only accepts a window between `100000` and `1000000` and silently ignores anything outside that range — so a setting of, say, `40000` did nothing at all while appearing to be configured. Cyrus now ignores an out-of-range value and says so in the log. ([#21](https://github.com/TropicalDog17/cyrus/pull/21))
- Read-only sessions (for example question/research or read-only-labelled work) can now search repository source code again. Previously these sessions could open files but were blocked from running `grep`/`rg`, so any attempt to search the codebase failed with a "permission denied" message even though the preset is meant for reading and grepping code. ([DEV-125](https://linear.app/devtrop/issue/DEV-125/permission-denied-for-grep-command-on-source-files))
...
### Changed
- Editing `~/.cyrus/config.json` while Cyrus is running now applies your changes more faithfully. Previously, "empty" values you set on disk — turning a trigger off (`false`), or clearing a tool allow-list to `[]` — could be silently ignored, leaving the old value in effect until the next full restart. Now a value you explicitly write to the file wins, so clearing a list actually clears it and switching a flag off actually takes effect on reload. Omitting a field entirely still preserves whatever value was already in use. A wider range of settings (including user access control and the global setup script) are now detected on live reload rather than requiring a restart.
