...
### Added
- **Cyrus now auto-picks a repository when routing is ambiguous.** When an issue matches no explicit routing rule (no `[repo=...]` tag, routing label, project, team, or catch-all) and more than one repository is configured, Cyrus asks a fast AI model to infer the best-fit repository from the issue title and description instead of immediately prompting you to choose. It only falls back to the "Please reply with an option" repository picker when it can't infer one confidently. On by default; disable it with `autoInferRepository: false` in `config.json`, and override the classifier model with `autoInferRepositoryModel`. ([DEV-117](https://linear.app/devtrop/issue/DEV-117/auto-set-project-for-issue-if-not-explicitly-configured))
- **LLMOps observability via Langfuse.** Cyrus can now stream a full trace of every agent session — each model request, tool call, token/cost metric, and prompt/response — to [Langfuse](https://langfuse.com/) for LLM observability. Turn it on by setting `LANGFUSE_PUBLIC_KEY` and `LANGFUSE_SECRET_KEY` (and optionally `LANGFUSE_HOST`) in your environment; when unset, nothing changes, and you can force it off with `CYRUS_TELEMETRY_DISABLED=1`. See [docs/CONFIG_FILE.md](docs/CONFIG_FILE.md#llmops-observability-langfuse). ([DEV-113](https://linear.app/devtrop/issue/DEV-113/setup-llmops-for-cyrus-agent-and-langfuse), [#11](https://github.com/TropicalDog17/cyrus/pull/11))
