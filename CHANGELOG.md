...
### Fixed
- **Linear tools are ready immediately instead of after a ~minute of searching.** When a session started, the agent often spent close to a minute running "tool search" round-trips against Linear's remote MCP server before it could read or update the issue — a visible stall at the top of every session. That happened because the SDK, seeing the large number of Linear tools, deferred them all behind an on-demand search to save context. Since every session needs to read and write Linear state anyway, Cyrus now loads the Linear tools (and its own `cyrus-tools`) up front, so they're available from the first turn with no search step. Rarely-used tool sets (docs, and optional Slack/Atlassian) stay deferred to keep startup context lean. ([DEV-140](https://linear.app/devtrop/issue/DEV-140/linear-search-functionality-is-not-straightforward))
