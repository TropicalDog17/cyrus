# GitHub App + webhooks (inbound)

Loaded from `cyrus-setup-github` only when the user opts into @mention responses.
Steps 1-4 (gh CLI auth + git identity) are in `SKILL.md` and have already run.

**CRITICAL: Never use `Read`, `Edit`, or `Write` on `~/.cyrus/.env` or any file
inside `~/.cyrus/`. Use only `Bash` (`grep`, `printf >>`, etc.) — secrets must
never be read into the conversation context.**
