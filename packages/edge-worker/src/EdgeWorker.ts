...
import { GitHubWorkflowService } from "./GitHubWorkflowService.js";
...
export class EdgeWorker extends EventEmitter {
	...
	private gitHubWorkflowService!: GitHubWorkflowService; // Owns the GitHub PR webhook workflow (extracted from EdgeWorker)
	...
	buildCollaborators(config: EdgeWorkerConfig): void {
		...
		const webhookRouterDeps: WebhookRouterDeps = {
			...
			handleGitHubComment: (event) =>
				this.gitHubWorkflowService.handleWebhook(event),
			handleGitHubPush: (payload) =>
				this.gitHubWorkflowService.handlePushWebhook(payload),
		...
		// GitHub PR workflow (extracted from EdgeWorker). Every dep is a
		// late-bound arrow closure over `this`. CRITICAL: handleMessage /
		// handleError bind to the heavy-body EdgeWorker handlers, and
		// dispatchGitHubEvent goes through the WebhookRouter — none of them route
		// back into gitHubWorkflowService.handleWebhook, so there is no recursion.
		this.gitHubWorkflowService = new GitHubWorkflowService({
			logger: this.logger,
			cyrusHome: this.cyrusHome,
			getFastifyInstance: () =>
				this.sharedApplicationServer.getFastifyInstance(),
			isIpValidationEnabled: () => this.webhookIpValidator.isEnabled(),
			getGitHubIpAllowlist: () =>
				this.webhookIpValidator.getAllowlist("github"),
			getPrReviewTrigger: () => this.config.prReviewTrigger,
			allRepositories: () => Array.from(this.repositories.values()),
			getAgentSessionManager: () => this.agentSessionManager,
			registerSessionRepository: (sessionId, repositoryId) => {
			...
			createGitWorktree: (issue, repositories) =>
				this.gitService.createGitWorktree(issue, repositories),
			...
			buildAgentRunnerConfig: (
				...
				this.sessionOrchestrator.buildAgentRunnerConfig(
					session,
					repository,
					sessionId,
					systemPrompt,
			...
			createRunnerForType: (runnerType, runnerConfig) =>
				this.sessionOrchestrator.createRunnerForType(runnerType, runnerConfig),
			savePersistedState: () => this.savePersistedState(),
			emitSessionStarted: (issueId, issue, repositoryId) => {
				this.emit("session:started", issueId, issue, repositoryId);
			},
			incrementActiveWebhookCount: () => {
				this.activeWebhookCount++;
			},
			decrementActiveWebhookCount: () => {
				this.activeWebhookCount--;
			},
			handleMessage: (message) => this.handleMessage(message),
			handleError: (error) => this.handleError(error),
			dispatchGitHubEvent: (event) =>
				this.webhookRouter.dispatchGitHubEvent(event),
		});
	...
	private async initializeComponents(): Promise<void> {
		...
		// 2. Register the GitHub event transport unconditionally (extracted)
		// It doesn't require repositories and must be available during onboarding
		// for webhook URL verification to succeed.
		this.gitHubWorkflowService.registerEventTransport();
