...
/**
 * Projects OMP ACP `session/update` notifications into the neutral
 * {@link AgentMessage} stream Cyrus consumes.
 *
 * OMP streams assistant text and reasoning as incremental chunks, and tool
 * calls as a `tool_call` (creation) followed by one or more `tool_call_update`
 * (status/output) notifications — the same ACP session/update protocol the
 * Codex adapter speaks, so the projection structure is shared. Tool names are
 * normalized to the Cyrus/Claude-style names the timeline renders; unknown
 * tools keep their real names (PR 1 wiring rule).
 */
export class OmpEventMapper {
	...
	constructor(private readonly opts: OmpEventMapperOptions) {}
	...
	handleUpdate(update: SessionUpdate): void {
		switch (update.sessionUpdate) {
			...
			case "plan":
				...
				return;
			default:
				// user_message_chunk (echoed prompt), plan_update/removed, mode/
				// config updates, usage — not projected into the timeline.
				return;
		}
	}
	...
	private handleToolCall(update: ToolCall): void {
		...
		// Some tool calls arrive already terminal with their output attached.
	...
	private handlePlan(plan: Plan): void {
		...
		// Project the plan as a TodoWrite tool_use so it renders as a checklist
		// in the Linear timeline, mirroring the Claude/Codex runners.
		this.planCounter += 1;
		this.pushToolUse(`omp-plan-${this.planCounter}`, "TodoWrite", { todos });
	}
 ...
 * appropriate rendering. OMP's built-ins surface under ACP kinds; anything
 * unmapped keeps its real (title-derived) name — unknown tools are never
...
export function toolNameFromKind(
	...
	switch (kind) {
		...
		case "think":
			return title || "Think";
		default:
			// switch_mode / other: keep the real (title-derived) name — unknown
			// tools are never collapsed into a generic label.
			return title || "Tool";
	}
}
...
export function normalizeToolInput(
	...
	return { input: String(rawInput), ...(title ? { tool: title } : {}) };
}
