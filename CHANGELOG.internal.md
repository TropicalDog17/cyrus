...
### Added
- **`packages/cyrus-loop`** — the compounding Verify → blind-gate → Learn loop, ported to TypeScript from the Python `agentic-pipeline` (zero Python at runtime). ~19 modules (paths, zod schemas, `runLog` durability core, spec/route/context/budgets/linearConventions, evidence ledger, citation-locked judge, blind gate, learn, metrics, integrate, capture) with their pytest suites ported verbatim to vitest (225 tests). Byte-parity with Python `json.dumps(sort_keys=True)` via `canonicalStringify`. The judge uses the raw `@anthropic-ai/sdk` with a forced tool call; durability uses fully-synchronous `runLog` + `proper-lockfile` (Node has no `F_FULLFSYNC`/`flock`). Loop data lives under `~/.cyrus/loop` (`AGENTIC_PIPELINE_DATA` override); config in `~/.cyrus/loop.json` (`CYRUS_LOOP_CONFIG` override), read independently to avoid `ConfigManager` merge-whitelist churn.
- **Loop wiring (Lane C).** New EdgeWorker bus events `prOpened` / `sessionComplete` / `verdictReached`; `PrMarkerHook` now returns the live PR facts and fires an `onPrOpened` callback (enriched with session context in `RunnerConfigBuilder`, re-emitted by EdgeWorker); `AgentSessionManager` emits `sessionComplete` on terminal (non-user-stop) sessions. `CyrusLoopAdapter` (new) subscribes the bus to `CyrusLoop`, owns per-issue gate state, posts the blind gate via `IIssueTrackerService.createComment`, and collects the human verdict through a new EdgeWorker prompt interceptor (diverts `/approve|/reject|/rework` before a session starts). Attached in `apps/cli` and `apps/f1`. A deterministic full-loop integration test (`CyrusLoopAdapter.integration.test.ts`, injected gh/Anthropic/worktree/Linear) is the e2e proof; see `apps/f1/test-drives/2026-07-07-cyrus-loop-trim-validation.md`.
