...
### Fixed
- **`EgressProxy.stop()` no longer hangs on in-flight connections.** The proxy now tracks every live socket (inbound client sockets and outbound tunnel sockets) and force-destroys them on `stop()`, and the plain TCP tunnel tears its upstream socket down on client close (not just on error). Previously an open CONNECT tunnel — or an upstream socket stuck mid-connect to an unreachable host in a no-egress sandbox — kept `server.close()` from ever calling back, wedging shutdown for the full OS TCP timeout. This surfaced as the `EgressProxy > updateNetworkPolicy > updates policy at runtime` test timing out the `afterEach` hook (30s) in CI. ([DEV-125](https://linear.app/devtrop/issue/DEV-125/permission-denied-for-grep-command-on-source-files))
