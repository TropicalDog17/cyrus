...
If two sessions target the same branch, the second does not get a tree of its
own: it reuses the first session's worktree, and both then edit the same files
with no lock. Nothing errors; it looks like an agent undoing its own work.
